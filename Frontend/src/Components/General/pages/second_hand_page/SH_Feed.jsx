...
import SHCard from "../../components/card/SHCard";
import Masonry from "react-masonry-css";
import { useEffect, useState } from "react";
import axios from "axios";
import DOCardSkeleton from "./DOCardSkeleton";
import ModalSHSAdd from "../../components/modal/ModalSHSAdd";
export default function SH_Feed({user,searchTerm}) {
  ...
  useEffect(() => {
    ...
    const getPosts = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/v1/second-hand-sales/retrieveAllItems`);
  ...
  return (
    <div className="LSfeed">
        ...
        <span className="page-title">Second-Hand Sales</span>
        <button className="plus-button" onClick={toggleModal}>+</button>
        </div>
        <ModalSHSAdd modal={modal} toggleModal={toggleModal} user={user} onItemShare={handleNewItemShare}/>
            ...
            : Posts.map(post => (
                <SHCard  key={post.id} post={post} currentUser={user}/>
              ))
          } 
